  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 4, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', borderBottom: '1px solid var(--border)' }}>
        <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, letterSpacing: 0.5, textTransform: 'uppercase' }}>MULTILINGUAL adaptation monitor</h3>
        <MoreHorizontal size={16} color="var(--text-secondary)" />
      </div>
      <div style={{ padding: '16px', flex: 1, display: 'flex', flexDirection: 'column', gap: 12 }}>
        
        <div style={{ marginBottom: 4 }}>
          <div style={{ fontSize: 16, fontWeight: 700 }}>LANGUAGE DETECTION: HINDI (Verified)</div>
          <div style={{ fontSize: 15, color: 'var(--text-secondary)' }}>[cite: Regional Accents: NCRTH]</div>
        </div>

        {data.map(item => (
          <div key={item.name} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ fontSize: 20 }}>{item.flag}</div>
            <div style={{ width: 60, fontSize: 16, fontWeight: 700 }}>{item.name}</div>
            ...
            <div style={{ width: 40, textAlign: 'right', fontSize: 15, fontWeight: 600 }}>{item.value}%</div>
          </div>
        ))}
