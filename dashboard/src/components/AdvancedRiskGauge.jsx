...
export default function AdvancedRiskGauge({ score, liveness }) {
  const rGauge = 70;
    ...
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 4, height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', padding: '16px' }}>
        ...
        <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', maxWidth: 180, marginTop: 4, fontSize: 10, fontWeight: 700, color: 'var(--text-secondary)' }}>
            <span>Low</span>
          ...
          <div style={{ marginTop: 12, background: 'var(--bg-base)', border: '1px solid var(--border)', borderRadius: 4, padding: 8, width: '100%', textAlign: 'left' }}>
            ...
            <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginTop: 4 }}>
              PASSIVE Liveness Score: {liveness ? Math.round(liveness * 100) : '--'}% [cite: {liveness > 0.8 ? 'SECURE' : 'WARNING'}]<br/>
              <span style={{ color: liveness && liveness < 0.8 ? 'var(--accent-rust)' : 'var(--text-secondary)' }}><AlertCircle size={10} style={{display:'inline', verticalAlign:'middle'}}/> Active Challenge Status: {liveness && liveness < 0.8 ? 'REQUIRED' : 'STANDBY'}</span>
            </div>
          </div>
        </div>

        {/* Right: Verification layers */}
        <div style={{ flex: 1.5, display: 'flex', flexDirection: 'column', gap: 12, paddingLeft: 16, borderLeft: '1px solid var(--border)' }}>
          <div>
            <div style={{ fontSize: 12, fontWeight: 800 }}>CALLER IDENTITY VERIFICATION</div>
            <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 8 }}>[cite: OPT-IN]</div>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ background: '#E5E7EB', padding: 6, borderRadius: '50%' }}><UserCircle color="#4B5563" size={16} /></div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600 }}>Voiceprint Match:</div>
                <div style={{ fontSize: 10, color: 'var(--text-secondary)' }}>[cite: NOT ENROLLED / NEW CALLER]</div>
              </div>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              <ShieldCheck color="#10B981" size={14} />
              <div style={{ fontSize: 11 }}>Accurate Liveness: <span style={{ color: '#10B981', fontWeight: 600 }}>91%</span></div>
            </div>
          </div>
          
          <div style={{ borderTop: '1px dotted var(--border)', paddingTop: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 800 }}>CALLER IDENTITY VERIFICATION</div>
            <div style={{ fontSize: 10, color: 'var(--text-secondary)', marginBottom: 8 }}>[cite: OPT-IN]</div>
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              <div style={{ background: '#D1FAE5', padding: 6, borderRadius: '50%' }}><Fingerprint color="#10B981" size={16} /></div>
              <div>
                <div style={{ fontSize: 11, fontWeight: 600 }}>Voiceprint Match:</div>
                <div style={{ fontSize: 10, color: 'var(--text-secondary)' }}>[cite: NOT ENROLLED / NEW CALLER]</div>
              </div>
